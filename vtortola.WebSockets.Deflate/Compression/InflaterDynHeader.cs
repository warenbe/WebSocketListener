...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
	internal class InflaterDynHeader
	{
		...
		private IEnumerable<bool> CreateStateMachine()
		{
			...
			if (litLenCodeCount > LITLEN_MAX) throw new ArgumentOutOfRangeException(nameof(litLenCodeCount));
			if (distanceCodeCount > DIST_MAX) throw new ArgumentOutOfRangeException(nameof(distanceCodeCount));
			if (metaCodeCount > META_MAX) throw new ArgumentOutOfRangeException(nameof(metaCodeCount));
			...
			while (index < dataCodeCount)
			{
				...
				else
				{
					...
					{
						if (index == 0)
							throw new IOException("Cannot repeat previous code length when no other code length has been read");
					...
					if (index + repeatCount > dataCodeCount)
						throw new IOException("Cannot repeat code lengths past total number of data code lengths");
			...
			if (codeLengths[256] == 0)
				throw new IOException("Inflater dynamic header end-of-block code missing");
		...
		/// </summary>
		/// <exception cref="IOException">If hader has not been successfully read by the state machine</exception>
		public InflaterHuffmanTree LiteralLengthTree
        {
            get
            {
                if (this.litLenTree != null)
                    ...
                    throw new IOException("Header properties were accessed before header had been successfully read");
            }
        }
